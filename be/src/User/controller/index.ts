...
import { Create, Login, ResetPassword, getUserByEmail, userDelete } from "../service";
...
export const forgetPassword = async(req:Request, res:Response)=>{
    ...
    try {
        ...
        if(!user) return res.status(404).json({message:"Email not found", status: 404})
        ...
        res.status(200).json({message: "password reset email sent successfully", status:200})
