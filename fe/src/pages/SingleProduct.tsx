...
const SingleProduct = () => {
  ...
  useEffect(() => {
    const orderedProducts = orderInfo?.filter(
      (item:Order) => item?.name === singleProduct?.name
    );
                    ...
                    orderedProducts.length > 0 ?(
                      orderedProducts?.map((item, indx) => (
                        <tr className="border">
                          ...
                          <td className="text-center capitalize">{item?.name}</td>
                          <td className="text-center capitalize">{item?.orderedId}</td>
                          <td className="text-center">{item?.noOfProducts}</td>
                          <td className="text-center capitalize">{item?.category}</td>
                        </tr>
                      ))

                    ):(
                      <tr>
                        <td className="py-32 text-center text-3xl text-gray-400 font-semibold" colSpan={5}>No Orders Yet</td>
                      </tr>
                    )
